...
begin
  ...
  if flag=true then
    begin
      ...
      Table.cells[2,i]:=floattostr(s);
      Table.cells[3,i]:=floattostr(v);
      i:=i+1;
      if i>3 then
  ...
  if SaveDialog1.Execute then
    if SaveDialog1.FileName <> '' then
      begin
        ...
        append(f);
        while Table.cells[0,j]<>'' do
          begin
            writeln(f,'№ '+form_cube.Table.cells[0,j]);
            writeln(f,'Сторона куба: '+form_cube.Table.cells[1,j]);
            writeln(f,'Площадь куба: '+form_cube.Table.cells[2,j]);
            writeln(f,'Объём куба: '+form_cube.Table.cells[3,j]);
            writeln(f);
          end;
        writeln(f,'-----------------------'+#13);
        closefile(f);
...
begin
  ...
  edit_side.text:='1';
  table.Cells[0,0]:='№';
  table.Cells[1,0]:='Сторона';
  table.Cells[2,0]:='Площадь';
  table.Cells[3,0]:='Объём';
  i:=1;
end;
